# Generated by the protocol buffer compiler.  DO NOT EDIT!
# sources: dota_gcmessages_client_guild_events.proto
# plugin: python-betterproto
from dataclasses import dataclass
from typing import List

import betterproto


class EGuildEventAuditAction(betterproto.Enum):
    k_EGuildEventAuditAction_Invalid = 0
    k_EGuildEventAuditAction_DevGrant = 1
    k_EGuildEventAuditAction_CompleteContract = 2
    k_EGuildEventAuditAction_CompleteChallenge = 3
    k_EGuildEventAuditAction_CompleteMatch_Winner = 4
    k_EGuildEventAuditAction_ChallengeProgress = 5
    k_EGuildEventAuditAction_CompleteMatch_Loser = 6
    k_EGuildEventAuditAction_WeeklyLeaderboard = 7
    k_EGuildEventAuditAction_ManualGrant = 8


class CMsgClientToGCRequestAccountGuildEventDataResponseEResponse(betterproto.Enum):
    k_eInternalError = 0
    k_eSuccess = 1
    k_eTooBusy = 2
    k_eDisabled = 3
    k_eTimeout = 4
    k_eInvalidEvent = 5
    k_eInvalidGuild = 6
    k_eNotMember = 7
    k_eInvalidGuildEvent = 8


class CMsgClientToGCRequestActiveGuildContractsResponseEResponse(betterproto.Enum):
    k_eInternalError = 0
    k_eSuccess = 1
    k_eTooBusy = 2
    k_eDisabled = 3
    k_eTimeout = 4
    k_eInvalidEvent = 5
    k_eInvalidGuild = 6
    k_eNotMember = 7
    k_eInvalidGuildEvent = 8


class CMsgClientToGCSelectGuildContractResponseEResponse(betterproto.Enum):
    k_eInternalError = 0
    k_eSuccess = 1
    k_eTooBusy = 2
    k_eDisabled = 3
    k_eTimeout = 4
    k_eInvalidEvent = 5
    k_eInvalidGuild = 6
    k_eNotMember = 7
    k_eInvalidGuildEvent = 8
    k_eInvalidContractID = 9
    k_eAlreadyAssigned = 10
    k_eInvalidContractSlot = 11
    k_eContractSlotLockedGuild = 12
    k_eContractSlotCooldown = 13
    k_eContractDuplicate = 14
    k_eContractSlotTimeError = 15
    k_eContractSlotLockedDotaPlus = 16


class CMsgClientToGCRequestActiveGuildChallengeResponseEResponse(betterproto.Enum):
    k_eInternalError = 0
    k_eSuccess = 1
    k_eTooBusy = 2
    k_eDisabled = 3
    k_eTimeout = 4
    k_eInvalidEvent = 5
    k_eInvalidGuild = 6
    k_eNotMember = 7
    k_eInvalidGuildEvent = 8


class CMsgClientToGCRequestGuildEventMembersResponseEResponse(betterproto.Enum):
    k_eInternalError = 0
    k_eSuccess = 1
    k_eTooBusy = 2
    k_eDisabled = 3
    k_eTimeout = 4
    k_eInvalidEvent = 5
    k_eInvalidGuild = 6
    k_eNotMember = 7
    k_eInvalidGuildEvent = 8


class CMsgClientToGCClaimLeaderboardRewardsResponseEResponse(betterproto.Enum):
    k_eInternalError = 0
    k_eSuccess = 1
    k_eTooBusy = 2
    k_eDisabled = 3
    k_eTimeout = 4
    k_eInvalidEvent = 5
    k_eInvalidGuild = 6
    k_eNotMember = 7
    k_eInvalidGuildEvent = 8
    k_eDoesNotQualify = 9
    k_eAlreadyClaimed = 10


@dataclass
class CMsgGuildContract(betterproto.Message):
    contract_id: int = betterproto.uint64_field(1)
    challenge_instance_id: int = betterproto.uint32_field(2)
    challenge_parameter: int = betterproto.uint32_field(3)
    challenge_timestamp: int = betterproto.uint32_field(4)
    assigned_account_id: int = betterproto.uint32_field(5)
    contract_flags: int = betterproto.uint32_field(6)


@dataclass
class CMsgGuildContractSlot(betterproto.Message):
    contract: "CMsgGuildContract" = betterproto.message_field(1)


@dataclass
class CMsgAccountGuildEventData(betterproto.Message):
    guild_points: int = betterproto.uint32_field(1)
    contracts_refreshed_timestamp: int = betterproto.uint32_field(2)
    contract_slots: List["CMsgGuildContractSlot"] = betterproto.message_field(3)
    completed_challenge_count: int = betterproto.uint32_field(4)
    challenges_refresh_timestamp: int = betterproto.uint32_field(5)
    guild_weekly_percentile: int = betterproto.uint32_field(6)
    guild_weekly_last_timestamp: int = betterproto.uint32_field(7)
    last_weekly_claim_time: int = betterproto.uint32_field(8)
    guild_current_percentile: int = betterproto.uint32_field(9)


@dataclass
class CMsgGuildActiveContracts(betterproto.Message):
    contracts_refreshed_timestamp: int = betterproto.uint32_field(1)
    contracts: List["CMsgGuildContract"] = betterproto.message_field(2)


@dataclass
class CMsgGuildChallenge(betterproto.Message):
    challenge_instance_id: int = betterproto.uint32_field(1)
    challenge_parameter: int = betterproto.uint32_field(2)
    challenge_timestamp: int = betterproto.uint32_field(3)
    challenge_progress: int = betterproto.uint32_field(4)
    challenge_flags: int = betterproto.uint32_field(5)


@dataclass
class CMsgGuildEventMember(betterproto.Message):
    account_id: int = betterproto.uint32_field(1)
    guild_points_earned: int = betterproto.uint32_field(2)


@dataclass
class CMsgClientToGCRequestAccountGuildEventData(betterproto.Message):
    guild_id: int = betterproto.uint32_field(1)
    event_id: "EEvent" = betterproto.enum_field(2)


@dataclass
class CMsgClientToGCRequestAccountGuildEventDataResponse(betterproto.Message):
    result: "CMsgClientToGCRequestAccountGuildEventDataResponseEResponse" = (
        betterproto.enum_field(1)
    )
    event_id: "EEvent" = betterproto.enum_field(2)
    event_data: "CMsgAccountGuildEventData" = betterproto.message_field(3)


@dataclass
class CMsgGCToClientAccountGuildEventDataUpdated(betterproto.Message):
    guild_id: int = betterproto.uint32_field(1)
    event_id: "EEvent" = betterproto.enum_field(2)
    update_flags: int = betterproto.uint32_field(3)
    guild_event_data: "CMsgAccountGuildEventData" = betterproto.message_field(4)
    contracts_updated: bool = betterproto.bool_field(5)


@dataclass
class CMsgClientToGCRequestActiveGuildContracts(betterproto.Message):
    guild_id: int = betterproto.uint32_field(1)
    event_id: "EEvent" = betterproto.enum_field(2)


@dataclass
class CMsgClientToGCRequestActiveGuildContractsResponse(betterproto.Message):
    result: "CMsgClientToGCRequestActiveGuildContractsResponseEResponse" = (
        betterproto.enum_field(1)
    )
    active_contracts: "CMsgGuildActiveContracts" = betterproto.message_field(2)
    active_challenges: "CMsgGuildChallenge" = betterproto.message_field(3)


@dataclass
class CMsgGCToClientActiveGuildContractsUpdated(betterproto.Message):
    guild_id: int = betterproto.uint32_field(1)
    event_id: "EEvent" = betterproto.enum_field(2)


@dataclass
class CMsgClientToGCSelectGuildContract(betterproto.Message):
    guild_id: int = betterproto.uint32_field(1)
    event_id: "EEvent" = betterproto.enum_field(2)
    contract_id: int = betterproto.uint64_field(3)
    contract_slot: int = betterproto.uint32_field(4)


@dataclass
class CMsgClientToGCSelectGuildContractResponse(betterproto.Message):
    result: "CMsgClientToGCSelectGuildContractResponseEResponse" = (
        betterproto.enum_field(1)
    )


@dataclass
class CMsgClientToGCRequestActiveGuildChallenge(betterproto.Message):
    guild_id: int = betterproto.uint32_field(1)
    event_id: "EEvent" = betterproto.enum_field(2)


@dataclass
class CMsgClientToGCRequestActiveGuildChallengeResponse(betterproto.Message):
    result: "CMsgClientToGCRequestActiveGuildChallengeResponseEResponse" = (
        betterproto.enum_field(1)
    )
    active_challenge: "CMsgGuildChallenge" = betterproto.message_field(2)


@dataclass
class CMsgGCToClientActiveGuildChallengeUpdated(betterproto.Message):
    guild_id: int = betterproto.uint32_field(1)
    event_id: "EEvent" = betterproto.enum_field(2)
    active_challenge: "CMsgGuildChallenge" = betterproto.message_field(3)


@dataclass
class CMsgClientToGCRequestGuildEventMembers(betterproto.Message):
    guild_id: int = betterproto.uint32_field(1)
    event_id: "EEvent" = betterproto.enum_field(2)


@dataclass
class CMsgClientToGCRequestGuildEventMembersResponse(betterproto.Message):
    result: "CMsgClientToGCRequestGuildEventMembersResponseEResponse" = (
        betterproto.enum_field(1)
    )
    members: List["CMsgGuildEventMember"] = betterproto.message_field(2)


@dataclass
class CMsgGuildLeaderboardCombinedResponse(betterproto.Message):
    event_id: "EEvent" = betterproto.enum_field(1)
    region: int = betterproto.uint32_field(2)
    last_updated: int = betterproto.uint32_field(3)
    guild_id: List[int] = betterproto.uint32_field(4)
    rank: List[int] = betterproto.uint32_field(5)
    current_percentile: List[int] = betterproto.uint32_field(6)
    weekly_percentile: List[int] = betterproto.uint32_field(7)
    points: List[int] = betterproto.uint32_field(8)


@dataclass
class CMsgClientToGCClaimLeaderboardRewards(betterproto.Message):
    guild_id: int = betterproto.uint32_field(1)
    event_id: "EEvent" = betterproto.enum_field(2)


@dataclass
class CMsgClientToGCClaimLeaderboardRewardsResponse(betterproto.Message):
    result: "CMsgClientToGCClaimLeaderboardRewardsResponseEResponse" = (
        betterproto.enum_field(1)
    )
    event_points: int = betterproto.uint32_field(2)
